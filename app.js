/* ============ ERROR REPORTER (diagnostic) ============ */
function __akaErr(msg){
  try{
    var b=document.getElementById("akaErr");
    if(!b){ b=document.createElement("div"); b.id="akaErr";
      b.style.cssText="position:fixed;top:0;left:0;right:0;z-index:9999;background:#8E2620;color:#fff;font:12px/1.5 monospace;padding:6px 10px;word-break:break-all";
      document.body.appendChild(b); }
    b.textContent="JS error: "+msg;
  }catch(_){}
}
window.addEventListener("error",function(e){ __akaErr((e.message||"unknown")+" (line "+(e.lineno||"?")+")"); });
...
}catch(e){ __akaErr("render: "+e.message); }
...
}catch(e){ __akaErr("order: "+e.message); }
...
}catch(e){ __akaErr("showcard: "+e.message); }
...
}catch(e){ __akaErr("phrases: "+e.message); }
...
}catch(e){ __akaErr("ask: "+e.message); }
...
}catch(e){ __akaErr("say: "+e.message); }
...
}catch(e){ __akaErr("omakase: "+e.message); }
...
}catch(e){ __akaErr("budget: "+e.message); }
...
}catch(e){ __akaErr("share: "+e.message); }
